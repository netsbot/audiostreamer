...
pub async fn get_m3u8(adam_id: String) -> Result<String, Box<dyn Error>> {
...
pub async fn get_token() -> Result<String, Box<dyn Error>> {
    ...
    let index_js_uri = js_re
        .find(&homepage_html)
        .ok_or("Could not find index JS URI")?
        .as_str();
    ...
    let token = token_re
        .find(&js_content)
        .ok_or("Could not find token in JS")?
        .as_str()
        .to_string();
...
pub async fn get_music_token() -> Result<String, Box<dyn Error>> {
