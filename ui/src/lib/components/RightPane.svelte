...
>
  <div
    bind:this={scroller}
    class="flex-1 overflow-y-auto no-scrollbar p-10 pt-4"
  >
      ...
      <div class="space-y-7 pb-28">
        ...
        {:else if lines.length === 0}
          <p class="text-zinc-500 text-sm">
            No lyrics loaded{error ? ` (${error})` : ""}
          </p>
        {:else}
          {#each lines as line, lineIndex}
            {@const lineDisplayEnd = getLineDisplayEnd(lineIndex)}
            {@const lineActive =
              playback.smoothTime >= line.start &&
              playback.smoothTime < lineDisplayEnd}
            <div
              class="display-synced-line mb-10 transition-all duration-700"
              style="filter: blur({lineActive ? '0px' : '2px'});"
            >
              <button
                class="line w-full text-left border-none bg-transparent p-0 cursor-pointer transition-all duration-500 whitespace-normal {lineActive
                  ? 'opacity-100'
              ...
              >
                ...
                >
                  <!-- Primary Vocals -->
                  <div
                    class="primary-vocals block text-[2rem] font-semibold tracking-tight leading-[1.2] whitespace-normal"
                  >
                    {#each line.words as word}
                      {#if word.syllables.some((s) => !s.isBackground)}
                        <span
                          class="group inline-flex whitespace-nowrap overflow-visible align-baseline"
                          class:mr-[0.22em]={word.hasTrailingSpace}
                        >
                          {#each word.syllables.filter((s) => !s.isBackground) as syllable}
                            {@const duration = Math.round(
                              (syllable.end - syllable.start) * 1000,
                            )}
                            {@const delay = Math.round(
                              (syllable.start - line.start) * 1000,
                            )}
                            ...
                            {@const progress = line.fullLineHighlight
                              ...
                              : Math.max(
                                  ...
                                  Math.min(
                                    100,
                                    ((playback.smoothTime - syllable.start) /
                                      safeDuration) *
                                      100,
                                  ),
                                )}
                            {@const isActive = line.fullLineHighlight
                              ? lineActive
                              : progress > 0 && progress < 100}

                            <div
                              class="main relative inline-grid place-items-center overflow-visible"
                            >
                              <span
                                ...
                                data-delay={delay}
                                style="--gradient-progress: {progress}%; --mask: {progress >=
                                100
                                  ? 'none'
                                  : `linear-gradient(to right, #000 0%, #000 var(--gradient-progress), transparent calc(var(--gradient-progress) + 40%))`}; --overlay-opacity: {progress >
                                0
                                  ? 1
                                  : 0};"
                              >
                  ...
                  {#if line.words.some( (w) => w.syllables.some((s) => s.isBackground), )}
                    <div
                      class="background-vocals flex flex-wrap items-center opacity-60 text-[1.4rem] font-medium text-zinc-400 mt-1 tracking-tight"
                    >
                      {#each line.words as word}
                        {#if word.syllables.some((s) => s.isBackground)}
                          <span
                            class="inline-flex"
                            class:mr-[0.15em]={word.hasTrailingSpace}
                          >
                            {#each word.syllables.filter((s) => s.isBackground) as syllable}
                              {@const duration = Math.round(
                                (syllable.end - syllable.start) * 1000,
    ...
    {:else}
      <div class="space-y-4 pb-28">
        <h3
          class="text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-4"
        >
          Upcoming
        </h3>
        ...
        {:else}
          {#each playback.activeQueue as track, i}
            ...
            <div
              class="group relative flex items-center gap-4 p-2 -mx-2 rounded-xl transition-all {isCurrent
                ? 'bg-white/10'
                : 'hover:bg-white/5'}"
            >
              <div
                class="relative size-12 shrink-0 overflow-hidden rounded-lg bg-zinc-800 shadow-md"
              >
                {#if track.metadata.artwork_url}
                  <img
                    src={track.metadata.artwork_url}
                    alt={track.metadata.title}
                    class="size-full object-cover {isCurrent
                      ? 'opacity-40'
                      : ''}"
                  />
                {/if}
              ...
              <div class="flex-1 min-w-0">
                <p
                  class="text-sm font-bold truncate {isCurrent
                    ? 'text-red-500'
                    : 'text-white'}"
                >
